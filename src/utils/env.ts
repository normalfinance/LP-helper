...
export function isAppNormalFinanceOrg({
	hostname,
}: {
	hostname: string;
}): boolean {
	return hostname === 'app.normalfinance.io';
}

export function isAppNormalFinanceStagingOrg({
	hostname,
}: {
	hostname: string;
}): boolean {
...
export function isBrowserRouterEnabled(): boolean {
	if (isProductionEnv()) {
		if (
			isAppNormalFinanceOrg(window.location) ||
			isAppNormalFinanceStagingOrg(window.location) ||
...
export function isSentryEnabled(): boolean {
	...
	if (isStagingEnv() && !isAppNormalFinanceStagingOrg(window.location))
		return false;
	if (isProductionEnv() && !isAppNormalFinanceOrg(window.location))
		return false;
